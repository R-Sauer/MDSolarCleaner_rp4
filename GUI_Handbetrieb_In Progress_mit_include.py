...
def move_step_motor_X_down():
    ...
    def motor_X_down():
        ...
        print('motor X down')
        # Check if the button is still pressed
        if btn_step_motor_etFrequencyInHzX_down_pressed.get():
            # Schedule the function to run again after 100 milliseconds
...
lbl_brush_motor_1.grid(row=12, column=1)
...
lbl_brush_motor_2.grid(row=14, column=1)

lbl_motor_2_drehrichtung = tk.Label(root, text="Drehrichtung:")
lbl_motor_2_drehrichtung .grid(row=16, column=2)
...
direction_menu_2.grid(row=16, column=3)
